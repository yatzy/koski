...
object ExamplesIB {
  ...
  val examples = List(
    Example("ib - final grades", "Oppija on suorittanut pre-IB vuoden ja IB-tutkinnon, IBO on vahvistanut arvosanat", Oppija(MockOppijat.ibFinal.toHenkilötiedotJaOid, List(opiskeluoikeus))),
    Example("ib - predicted grades", "Oppija on suorittanut pre-IB vuoden ja IB-tutkinnon, IBO ei ole vahvistanut arvosanoja", Oppija(MockOppijat.ibPredicted.toHenkilötiedotJaOid, List(opiskeluoikeusPredictedGrades)))
  )
}
